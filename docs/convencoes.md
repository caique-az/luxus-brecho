...
- Respostas seguem o envelope **plano** `{ "success": bool, "message": str, ... }`: os dados de domínio ficam no topo, ao lado de `success` (não aninhados em `data`). Monte sempre com os helpers `ok()` / `err()` de `app/utils/responses.py`, nunca com `jsonify` cru — assim `success` serve de discriminador em todo endpoint e a mensagem de erro fica sempre em `message` (a chave `error` foi abolida, inclusive nos erros de autenticação dos decorators JWT). Erros de validação trazem `errors: { campo: motivo }`. Exceção: endpoints que devolvem uma **lista pura** (ex.: `/categories/summary`) não passam pelo envelope.
