...
| Pedidos / Checkout | posse ou admin em todas as rotas | ❌ Checkout usa `fetch` cru sem Bearer ([FE-03](#fe-03)) | ❌ checkout fora do fluxo principal ([MB-05](#mb-05)) |
...
|----|--------|-------|--------------------|---------------|
| <a id="be-01"></a>BE-01 | **Envelope `{success}` não padronizado** (padronização deferida na Fase 6). Coexistem três formatos: maioria responde `{message: ...}`; `jwt_service.py` e `images_controller.py` respondem `{'error': ...}`; e `{success: ...}` aparece só em três pontos — health, os error handlers globais (404/500/405/413) e o ramo `ValidationError` de `list_products`. Superfície estimada: ~266 `jsonify` em 12 arquivos. Recomendação registrada: criar helpers `ok()/err()` em `utils/` e migrar incrementalmente (nunca num sweep único — risco de acoplamento com parsing do web/mobile). | `backend/app/services/jwt_service.py`, `backend/app/controllers/images_controller.py`, `backend/app/__init__.py`, `backend/app/routes/products_routes.py` | Um 401 de auth retorna `{"error"}`, enquanto um 404 de produto retorna `{"message"}` — clientes precisam tratar os dois | 2026-07-01 |
...
| <a id="mb-08"></a>MB-08 | **Build Android quebrado (`react-native-worklets` incompatível)**: `react-native-worklets@0.5.1` é dependência **direta**, mas exige RN ≥ 0.74; o app roda em `react-native@0.72.10` (Expo 49). O Gradle reprova em `:react-native-worklets:assertMinimalReactNativeVersionTask` ("React Native 0.72.10 version is not compatible with Worklets 0.5.1"). O pacote é redundante nesse setup — `react-native-reanimated@~3.3.0` já embute os worklets; a correção provável é remover a dependência direta. | `mobile/package.json:47` (worklets), `:40` (RN), `:42` (reanimated); CI em `.github/workflows/mobile-build.yml` (job "Build Android APK", dispara em `pull_request` p/ main/dev sem filtro de `paths`) | Build nativo nunca compila → sem APK/instalável de teste; como o job roda em todo PR, cada PR nasce com o check vermelho (visto no PR #19, run 28912176724). Amplifica [MB-07](#mb-07) | 2026-07-07 |
