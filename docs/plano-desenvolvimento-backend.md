...
- [x] **Remover a segunda `create_app()` obsoleta** (`controllers/__init__.py`) — hoje tem `DEBUG=True`, banco fixo `luxus_brecho`, `app.mongo` apontando para o database (não o client) e só 2 blueprints. É uma sombra perigosa da factory real. Deixar o arquivo como marcador de pacote vazio. — **Concluído:** confirmado que nenhum entrypoint (`index.py`/`run.py`/`conftest.py`) a importava; arquivo virou só docstring de marcador de pacote.
- [x] **Eliminar código morto de `products_controller.py`** — `products_routes.py` reimplementa tudo inline e as cópias já divergiram. Manter uma implementação só (controller fino) e apagar a duplicata. — **Concluído:** `products_controller.py` era código morto (nenhum import no repo); a versão viva é a de `products_routes.py`, que — ao contrário do controller — tem os `@admin_required` e a limpeza de imagem no delete. Deletei o controller. (Extrair um "controller fino" a partir das rotas fica como refinamento opcional; a duplicata insegura já foi eliminada.)
- [x] **Centralizar o guard `db is None`** (copiado ~45×, com 3 mensagens diferentes) num `@app.before_request` ou decorator `require_db`, retornando 503 canônico. — **Concluído:** decorator `require_db` em `utils/db.py`; aplicado às 46 views (users/order/cart/favorites/categories + products_routes), removendo o guard inline e unificando as 3 mensagens (`banco de dados indisponível`/`Banco de dados indisponível`/`database unavailable`) num único 503 canônico. `health_controller` fica de fora (testa `.mongo` com corpo próprio).
- [~] **Extrair helpers duplicados para `utils/`:** `_serialize` (6 cópias, uma expõe `_id` cru), `get_next_id`/`get_next_sequence` (4 cópias), parsing de paginação (6 cópias — `list_users` sem clamp permite `page_size` ilimitado, `users_controller.py:43-44`). — **Parcial:** `utils/serialization.serialize_doc` unifica os `_serialize` (categories/products_routes/favorites passam a delegar) e **corrige o leak de `_id`** de favoritos (controller + `favorite_model`); `utils/pagination.parse_pagination` criado e aplicado a `list_users`, **fechando o clamp ausente** (page_size limitado a 100). *Pendente (baixo valor / risco):* migrar as outras 5 chamadas de paginação (já fazem clamp correto) e unificar `get_next_id`/`get_next_sequence` (risco de bug de geração de ID; as 4 cópias funcionam).
- [x] **Rate limiter fora de `memory://`** (`__init__.py:97`) — em múltiplos workers/serverless (Vercel) os contadores multiplicam e zeram a cada restart, enfraquecendo a proteção de brute-force do login. Usar storage compartilhado (Redis) em produção. — **Concluído (código):** `storage_uri` agora vem de `RATELIMIT_STORAGE_URI` (default `memory://`, adicionado ao `.env.example`); loga *warning* quando cai no `memory://`. Provisionar o Redis e definir a env em produção é passo de deploy/ops.
- [~] **Eficiência:** `create_order` faz N+1 (usar `$in` + `update_many`); `create_product_with_image` faz upload duplo sempre (obter o id sequencial real antes e subir uma vez); SMTP síncrono no caminho da requisição (signup/confirmação/reset → enviar assíncrono); lookups por `token_confirmacao`/`reset_token` sem índice (adicionar índices esparsos). — **Parcial:** `create_order` resolve os produtos numa única query `$in` (leitura) e marca todos como vendido com `update_many` (escrita), no caminho transacional e no fallback; índices esparsos de `token_confirmacao`/`reset_token` adicionados em `ensure_users_collection`. *Pendente:* upload duplo em `create_product_with_image` (risco médio, Supabase) e SMTP assíncrono (precisa de fila/thread — decisão arquitetural).
- [x] **Registro de blueprints não deve engolir `ImportError`** (`__init__.py:255`) — hoje um erro num arquivo de rotas remove a feature inteira só com um `print`. Falhar rápido no startup para blueprints obrigatórios. — **Concluído:** o loop agora re-lança qualquer falha como `RuntimeError` com contexto (módulo/blueprint), abortando o startup em vez de servir uma API mutilada.
- [ ] **Padronizar respostas com o campo `success`** — os controllers omitem `success` sistematicamente, contrariando o padrão do CLAUDE.md (`{ "success": bool, "message": str, ... }`). — **Deferido:** superfície de ~240 respostas `jsonify` em 9 arquivos; é aditivo mas de alto churn e com risco de acoplamento com frontend/mobile (parsing estrito). Os error handlers (404/500/405/413) já retornam `success:false`. Recomendação: criar helpers `ok()/err()` em `utils` e migrar incrementalmente, não num sweep único.
- [x] **Escapar/tratar regex em `list_users`** (`users_controller.py:60-64`) — `{"$regex": search}` com input não escapado permite ReDoS (exploração limitada a admin autenticado). Escapar o termo ou usar índice de texto. — **Concluído:** o termo passa por `re.escape` antes do `$regex` (tratado como literal).
